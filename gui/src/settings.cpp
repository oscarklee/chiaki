...
QMap<int, Qt::Key> Settings::GetControllerMapping()
{
    ...
    for(auto chiaki_button : chiaki_buttons)
    {
        ...
        if(settings.contains("keymap/" + button_name)) {
            QKeySequence keySequence = QKeySequence::fromString(settings.value("keymap/" + button_name).toString());
            result[static_cast<int>(chiaki_button)] = Qt::Key(keySequence[0].key());
        }
    }
